    ...
    return (
        <StyledSavedForLater>
            <Container className='container'>
                ...
                <div className='cards'>
                    {SAVED_FOR_LATERS.map((item) => {
                        return <div className="card" key={item.id}>
                            <div className='image' style={{ position: "relative", width: "225px", height: "225px" }}>
                                <CustomImage src={item.src} alt={item.title} fill={true} objectFit="contain" />
                            </div>
                            <p className='price'>{item.price}</p>
                            <p className='title'>{item.title}</p>
                            <Button bgColor="#fff" color="#0D6EFD" width="157px" height="40px" padding="9px 12px 9px 9px">
